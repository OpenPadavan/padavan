		...
		if(n<=15) {
			...
			if(addr_is_reserved(&lan_addr->ext_ip_addr)) {
				if (GETFLAG(ALLOWPRIVATEIPV4MASK)) {
					syslog(LOG_DEBUG, "IGNORED : option ext_ip address contains reserved / private address : %s", lan_addr->ext_ip_str);
				} else {
					/* error */
...
static void update_disable_port_forwarding(void)
{
	...
	if (r < 0) {
		...
		if (!disable_port_forwarding && reserved) {
			if (GETFLAG(ALLOWPRIVATEIPV4MASK)) {
				syslog(LOG_DEBUG, "IGNORED : Reserved / private IP address %s on ext interface %s", if_addr, ext_if_name);
			} else {
#if 0
...
init(int argc, char * * argv, struct runtime_vars * v)
{
	...
	if (use_ext_ip_addr) {
		...
		if (addr_is_reserved(&addr)) {
			if (GETFLAG(ALLOWPRIVATEIPV4MASK)) {
				syslog(LOG_DEBUG, "IGNORED : option ext_ip contains reserved / private address %s, not public routable", use_ext_ip_addr);
			} else {
