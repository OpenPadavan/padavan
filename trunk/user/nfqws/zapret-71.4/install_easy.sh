...
ws_opt_validate()
{
	# NOTE FOR REDISTRIBUTORS OF COPYPASTED SCRIPTS
	# THIS CODE WAS MADE SPECIALLY FOR YOU SO THAT YOU DO NOT POST BAD RECIPES TO THE INTERNET.
	# IF YOU WANT TO DELETE IT AND WRITE INSTRUCTIONS ON HOW TO DELETE IT, YOU ARE DOING A DISSERVICE. WRITE A custom script INSTEAD.
	# A custom script IS A FILE THAT YOU ONLY NEED TO COPY TO THE RIGHT DIRECTORY AND IT WILL DO THE SAME THING, BUT EFFECTIVELY.
	# IPSET FILTERING INSIDE THE KERNEL IS FAR MORE EFFECTIVE THAN PASSING ALL PACKETS TO nfqws AND FILTERING THERE
	# --ipset EXISTS ONLY FOR WINDOWS AND LINUX SYSTEMS WITHOUT ipset (FOR EXAMPLE, Android).
	# AND ONLY FOR THIS REASON IT IS NOT COMPLETELY REMOVED FROM THE LINUX VERSION
